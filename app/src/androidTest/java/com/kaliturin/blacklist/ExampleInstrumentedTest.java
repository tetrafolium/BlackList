...
public void privatePhoneNumber() throws Exception {
	...
	for (String number : privateNumbers) {
		assertTrue("number = {" + number + "} is not private",
		           ContactsAccessHelper.isPrivatePhoneNumber(number));
	}

	String[] normalNumbers = {"-2-2",
		                  "-2 -2",
		                  "+01234567890",
		                  "+0 123 456 78 90",
		                  "0 123 456 78 90",
	...
	for (String number : normalNumbers) {
		assertFalse("number = {" + number + "} is private",
		            ContactsAccessHelper.isPrivatePhoneNumber(number));
	}
}
...
public void normalizePhoneNumber() throws Exception {
	...
	String[] notNormalizedNumbers = {
		"+0 123 456 78 90",    "+0 (123) 456-78-90", "+0-123-456-78-90",
		"+ 0 (123) 456 78 90", " +0 123 456 78-90 ", "\n+ 0123 456 78 90\n"
	};
	for (String number : notNormalizedNumbers) {
		assertEquals("number = {" + number + "} cannot be normalized",
		             normalizedNumber,
...
public void smsSubscriptionManager() throws Exception {
	...
	if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
		List<SubscriptionInfo> list =
			SubscriptionHelper.getSubscriptions(context);
		if (list != null) {
