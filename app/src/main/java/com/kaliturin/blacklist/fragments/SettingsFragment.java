...
public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
	...
	Permissions.notifyIfNotGranted(getContext(),
...
// Is used for getting result of ringtone picker dialog & default sms app
// dialog
@Override
...
private void loadListViewItems(final int listPosition) {
	...
	if (isDefaultSmsAppAvailable) {
		...
		adapter.addCheckbox(R.string.Default_SMS_app,
		                    R.string.Set_as_default_SMS_app, isDefaultSmsApp,
	...
	if (SubscriptionHelper.isAvailable()) {
		...
		adapter.addButton(
			getString(R.string.Chosen_SIM), getCurrentSimName(),
			new View.OnClickListener() {
				@Override
				public void onClick(View v) {
				        // check permissions
				        if (Permissions.notifyIfNotGranted(
						    getContext(), Permissions.READ_PHONE_STATE)) {
				                return;
					}
	...
	if (isDefaultSmsApp) {
		...
		adapter.addCheckbox(R.string.All_SMS, R.string.Block_all_SMS,
		                    Settings.BLOCK_ALL_SMS);
		adapter.addCheckbox(R.string.Black_list,
		                    R.string.Block_SMS_from_black_list,
		                    Settings.BLOCK_SMS_FROM_BLACK_LIST);
		adapter.addCheckbox(R.string.Contacts_list,
		                    R.string.Block_SMS_not_from_contacts,
		                    Settings.BLOCK_SMS_NOT_FROM_CONTACTS);
		adapter.addCheckbox(R.string.SMS_list,
		                    R.string.Block_SMS_not_from_SMS_list,
		                    Settings.BLOCK_SMS_NOT_FROM_SMS_CONTENT);
		adapter.addCheckbox(R.string.Private_numbers,
		                    R.string.Block_SMS_from_private,
		...
		adapter.addCheckbox(R.string.Status_bar,
		                    R.string.Notify_in_status_bar_blocked_SMS,
		                    Settings.BLOCKED_SMS_STATUS_NOTIFICATION,
		                    new DependentRowOnClickListener());
		adapter.addCheckbox(R.string.Sound,
		                    R.string.Notify_with_sound_blocked_SMS,
		                    Settings.BLOCKED_SMS_SOUND_NOTIFICATION,
		                    new RingtonePickerOnClickListener(BLOCKED_SMS));
		adapter.addCheckbox(R.string.Vibration,
		                    R.string.Notify_with_vibration_blocked_SMS,
	...
	adapter.addCheckbox(R.string.Vibration,
	                    R.string.Notify_with_vibration_received_SMS,
	...
	adapter.addCheckbox(R.string.All_calls, R.string.Block_all_calls,
	                    Settings.BLOCK_ALL_CALLS);

	adapter.addCheckbox(R.string.Black_list,
	                    R.string.Block_calls_from_black_list,
	                    Settings.BLOCK_CALLS_FROM_BLACK_LIST);
	adapter.addCheckbox(R.string.Contacts_list,
	                    R.string.Block_calls_not_from_contacts,
	                    Settings.BLOCK_CALLS_NOT_FROM_CONTACTS);
	adapter.addCheckbox(R.string.SMS_list,
	                    R.string.Block_calls_not_from_SMS_list,
	                    Settings.BLOCK_CALLS_NOT_FROM_SMS_CONTENT);
	adapter.addCheckbox(R.string.Private_numbers,
	                    R.string.Block_calls_from_private,
	...
	adapter.addCheckbox(R.string.Status_bar,
	                    R.string.Notify_in_status_bar_blocked_call,
	                    Settings.BLOCKED_CALL_STATUS_NOTIFICATION,
	                    new DependentRowOnClickListener());
	...
	adapter.addCheckbox(R.string.Vibration,
	                    R.string.Notify_with_vibration_blocked_call,
	                    Settings.BLOCKED_CALL_VIBRATION_NOTIFICATION,
	                    new DependentRowOnClickListener());

	// app interface
	adapter.addTitle(R.string.App_interface);
	adapter.addCheckbox(R.string.Text_folding,
	                    R.string.Journal_SMS_text_folding,
	...
	adapter.addCheckbox(R.string.UI_theme_dark, 0, Settings.UI_THEME_DARK,
	                    new View.OnClickListener() {
			@Override
	...
	// export DB file
	adapter.addButton(
		R.string.Export_data, R.string.Write_data_into_external,
		new View.OnClickListener() {
			@Override
			public void onClick(View v) {
			        // check permissions
			        if (Permissions.notifyIfNotGranted(
					    getContext(), Permissions.WRITE_EXTERNAL_STORAGE)) {
			                return;
				}
			        // open the dialog for getting the exporting DB file path
			        showFilePathDialog(
					R.string.Export_data, new TextView.OnEditorActionListener() {
					@Override
	...
	// import DB file
	adapter.addButton(
		R.string.Import_data, R.string.Load_data_from_external,
		new View.OnClickListener() {
			@Override
			public void onClick(View v) {
			        // check permissions
			        ...
			        showFilePathDialog(
					R.string.Import_data, new TextView.OnEditorActionListener() {
					@Override
					public boolean onEditorAction(TextView textView, int actionId,
					                              KeyEvent event) {
					        // import data file
...
private Uri getRingtoneUri(int requestCode) {
	...
	case BLOCKED_CALL:
		uriString =
			Settings.getStringValue(getContext(), Settings.BLOCKED_CALL_RINGTONE);
		break;
	case BLOCKED_SMS:
		uriString =
			Settings.getStringValue(getContext(), Settings.BLOCKED_SMS_RINGTONE);
		break;
	case RECEIVED_SMS:
		uriString =
			Settings.getStringValue(getContext(), Settings.RECEIVED_SMS_RINGTONE);
		break;
	}
	...
	if (isAdded()) {
		if (adapter.isRowChecked(view)) {
			...
			intent.putExtra(RingtoneManager.EXTRA_RINGTONE_TITLE,
			                getString(R.string.Ringtone_picker));
			intent.putExtra(RingtoneManager.EXTRA_RINGTONE_EXISTING_URI,
			                getRingtoneUri(requestCode));
...
public void onClick(View view) {
	...
	if (!checked) {
		...
		case Settings.BLOCKED_SMS_STATUS_NOTIFICATION:
			...
			adapter.setRowChecked(Settings.BLOCKED_SMS_VIBRATION_NOTIFICATION,
			                      false);
			break;
		case Settings.BLOCKED_CALL_STATUS_NOTIFICATION:
			adapter.setRowChecked(Settings.BLOCKED_CALL_SOUND_NOTIFICATION,
			                      false);
			adapter.setRowChecked(Settings.BLOCKED_CALL_VIBRATION_NOTIFICATION,
			                      false);
			break;
		}
	} else {
...
private void
showFilePathDialog(@StringRes int titleId,
...
private boolean exportDataFile(String dstFilePath) {
	if (!Permissions.isGranted(getContext(),
	                           Permissions.WRITE_EXTERNAL_STORAGE)) {
		return false;
	}
...
private void showSimChoosingDialog() {
	...
	List<SubscriptionInfo> list =
		SubscriptionHelper.getSubscriptions(getContext());
	if (list != null) {
			...
			if (name != null) {
				dialog.addItem(0, name, info, new View.OnClickListener() {
						...
						public void onClick(View v) {
						        ...
						        if (id != null) {
						                Settings.setIntegerValue(getContext(),
						                                         Settings.SIM_SUBSCRIPTION_ID, id);
						                reloadListViewItems();
							}
						}
					});
	...
	dialog
	.addItem(R.string.By_default,
	         new View.OnClickListener() {
			@Override
			public void onClick(View v) {
			        Settings.setIntegerValue(getContext(),
			                                 Settings.SIM_SUBSCRIPTION_ID, -1);
			        reloadListViewItems();
			}
		})
	.show();
