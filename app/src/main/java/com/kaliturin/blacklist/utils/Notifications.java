...
public class Notifications {
  ...
  public static void onCallBlocked(Context context, String address) {
    if (!Settings.getBooleanValue(context,
    ...
    Uri ringtone =
        getRingtoneUri(context, Settings.BLOCKED_CALL_SOUND_NOTIFICATION,
                       Settings.BLOCKED_CALL_RINGTONE);
    boolean vibration = Settings.getBooleanValue(
        context, Settings.BLOCKED_CALL_VIBRATION_NOTIFICATION);
    notify(context, address, message, message, icon, action, ringtone,
           vibration);
  }
  ...
  public static void onSmsBlocked(Context context, String address) {
    ...
    String action = MainActivity.ACTION_JOURNAL;
    Uri ringtone =
        getRingtoneUri(context, Settings.BLOCKED_SMS_SOUND_NOTIFICATION,
                       Settings.BLOCKED_SMS_RINGTONE);
    boolean vibration = Settings.getBooleanValue(
        context, Settings.BLOCKED_SMS_VIBRATION_NOTIFICATION);
    notify(context, address, message, message, icon, action, ringtone,
           vibration);
  }

  // Notification on SMS received
  public static void onSmsReceived(Context context, String address,
                                   String smsBody) {
    ...
    Uri ringtone =
        getRingtoneUri(context, Settings.RECEIVED_SMS_SOUND_NOTIFICATION,
                       Settings.RECEIVED_SMS_RINGTONE);
    boolean vibration = Settings.getBooleanValue(
        context, Settings.RECEIVED_SMS_VIBRATION_NOTIFICATION);
    notify(context, address, message, smsBody, icon, action, ringtone,
           vibration);
  }

  public static void onSmsDelivery(Context context, String address,
                                   String message) {
    if (!Settings.getBooleanValue(context,
  ...
  private static void notify(Context context, String title, String message,
                             String ticker, @DrawableRes int icon,
  ...
  // Returns notification ringtone uri (if settings do not allow it - returns
  // null)
  private static Uri getRingtoneUri(Context context,
                                    String notificationProperty,
