 ...
 * Is needed since API19, where only default SMS app can stop SMS from
 * receiving.
 */
...
public static void enableSMSReceiving(Context context, boolean enable) {
	...
	ComponentName componentName =
		new ComponentName(context, SMSBroadcastReceiver.class);
...
@TargetApi(19)
public static void askForDefaultAppChange(Fragment fragment,
                                          int requestCode) {
	if (!isAvailable())
		return;
	...
	if (isDefault(context)) {
		...
		packageName = Settings.getStringValue(
			context, Settings.DEFAULT_SMS_APP_NATIVE_PACKAGE);
	} else {
		...
		if (nativePackage != null) {
			Settings.setStringValue(
				context, Settings.DEFAULT_SMS_APP_NATIVE_PACKAGE, nativePackage);
		}
	}
...
@TargetApi(19)
private static void askForDefaultAppChange(Fragment fragment,
