...
public class ContactsAccessHelper {
  ...
  @Nullable
  public Cursor getContacts(Context context, ContactSourceType sourceType,
    ...
    case FROM_BLACK_LIST: {
      ...
      if (db != null) {
        return db.getContacts(DatabaseAccessHelper.Contact.TYPE_BLACK_LIST,
                              filter);
      }
    }
    case FROM_WHITE_LIST: {
      DatabaseAccessHelper db = DatabaseAccessHelper.getInstance(context);
      if (db != null) {
        return db.getContacts(DatabaseAccessHelper.Contact.TYPE_WHITE_LIST,
                              filter);
      }
    }
    }
  ...
  private ContactCursorWrapper getContacts(@Nullable String filter) {
    ...
    Cursor cursor = contentResolver.query(
        ...
        Contacts.IN_VISIBLE_GROUP + " != 0 AND " + Contacts.HAS_PHONE_NUMBER +
            " != 0 AND " + Contacts.DISPLAY_NAME + " IS NOT NULL AND " +
            Contacts.DISPLAY_NAME + " LIKE ? ",
        new String[] {filter}, Contacts.DISPLAY_NAME + " ASC");
  ...
  private ContactCursorWrapper getContactCursor(String number) {
    ...
    Cursor cursor = contentResolver.query(
        lookupUri, new String[] {Contacts._ID, Contacts.DISPLAY_NAME}, null,
        null, null);
  ...
  private class ContactCursorWrapper
      extends CursorWrapper implements ContactSource {
  ...
  private static class ContactNumberCursorWrapper extends CursorWrapper {
    ...
    String getNumber() { return getString(NUMBER); }
  }
  ...
  private ContactNumberCursorWrapper getContactNumbers(long contactId) {
    Cursor cursor =
        contentResolver.query(Phone.CONTENT_URI, new String[] {Phone.NUMBER},
  ...
  private static final Uri URI_CONTENT_SMS_INBOX =
      Uri.parse("content://sms/inbox");
  private static final Uri URI_CONTENT_SMS_CONVERSATIONS =
      Uri.parse("content://sms/conversations");
  private static final Uri URI_CONTENT_CALLS =
  ...
  public boolean containsNumberInSMSContent(Context context,
                                            @NonNull String number) {
    if (!Permissions.isGranted(context, Permissions.READ_SMS)) {
      return false;
    }

    // We cannot select passed phone number simply by using query because some
    // stored numbers may be not normalized. So we select all the unique numbers
    // first, normalize them, and then search for our number.
    Cursor cursor = contentResolver.query(
        URI_CONTENT_SMS, new String[] {"DISTINCT " + ADDRESS},
        ADDRESS + " IS NOT NULL) GROUP BY (" + ADDRESS, null, DATE + " DESC");
  ...
  @Nullable
  private ContactFromSMSCursorWrapper
  getContactsFromSMSList(@Nullable String filter) {
    filter = (filter == null ? "" : filter.toLowerCase());

    // filter by address (number) if person (contact id) is null
    Cursor cursor = contentResolver.query(
        URI_CONTENT_SMS, new String[] {"DISTINCT " + ID, ADDRESS, PERSON},
        ADDRESS + " IS NOT NULL "
            + ") GROUP BY (" + ADDRESS,
        null, DATE + " DESC");
    ...
    if (validate(cursor)) {
      ...
      MatrixCursor matrixCursor =
          new MatrixCursor(new String[] {ID, ADDRESS, PERSON});
  ...
  private class ContactFromSMSCursorWrapper
      extends CursorWrapper implements ContactSource {
  ...
  @Nullable
  private ContactFromCallsCursorWrapper
  getContactsFromCallsLog(@Nullable String filter) {
    filter = (filter == null ? "%%" : "%" + filter + "%");
    ...
    Cursor cursor = contentResolver.query(
        ...
        Calls.NUMBER + " IS NOT NULL AND (" + Calls.CACHED_NAME +
            " IS NULL AND " + // leave out private numbers
            Calls.NUMBER + " NOT LIKE '-%' AND " + Calls.NUMBER +
            " LIKE ? OR " + Calls.CACHED_NAME + " LIKE ? )",
        new String[] {filter, filter}, Calls.DATE + " DESC");
    ...
    return (validate(cursor) ? new ContactFromCallsCursorWrapper(cursor)
                             : null);
  }

  // Contact from calls cursor wrapper
  private class ContactFromCallsCursorWrapper
      extends CursorWrapper implements ContactSource {
    private final int ID;
    private final int NUMBER;
    private final int NAME;
  ...
  public boolean deleteLastRecordFromCallLog(Context context, String number,
                                             long duration) {
    ...
    int count = contentResolver.delete(URI_CONTENT_CALLS, ID + " = ? ",
                                       new String[] {String.valueOf(id)});

    return (count > 0);
  }

  // Returns last call record from the Call log that was written since
  // "duration" time
  private long getLastRecordIdFromCallLog(Context context, String number,
                                          long duration) {
    if (!Permissions.isGranted(context, Permissions.READ_CALL_LOG)) {
      return -1;
    }

    // We should not search call just by number because it can be not
    // normalized. Therefore select all records have been written since passed
    // time duration, normalize every number and then compare.
    long time = System.currentTimeMillis() - duration;

    Cursor cursor = contentResolver.query(
        URI_CONTENT_CALLS, new String[] {Calls._ID, Calls.NUMBER},
        Calls.DATE + " > ? ", new String[] {String.valueOf(time)},
        Calls.DATE + " DESC");
  ...
  public class SMSConversation {
    ...
    SMSConversation(int threadId, long date, String person, String number,
                    String snippet, int unread) {
  ...
  public SMSConversationWrapper getSMSConversations(Context context) {
    ...
    Cursor cursor =
        contentResolver.query(URI_CONTENT_SMS_CONVERSATIONS,
  ...
  @Nullable
  private SMSConversation getSMSConversationByThreadId(Context context,
                                                       int threadId) {
    if (!Permissions.isGranted(context, Permissions.READ_SMS) ||
        !Permissions.isGranted(context, Permissions.READ_CONTACTS)) {
      return null;
    ...
    SMSMessageCursorWrapper cursor =
        getSMSMessagesByThreadId(context, threadId, true, 1);
    if (cursor != null) {
      SMSMessage sms = cursor.getSMSMessage(true);
      smsConversation = new SMSConversation(threadId, sms.date, sms.person,
                                            sms.number, sms.body, unread);
      cursor.close();
    }
  ...
  @Nullable
  private SMSMessageCursorWrapper
  getSMSMessagesByThreadId(Context context, int threadId, boolean desc,
                           int limit) {
    if (!Permissions.isGranted(context, Permissions.READ_SMS) ||
        !Permissions.isGranted(context, Permissions.READ_CONTACTS)) {
      return null;
    ...
    Cursor cursor = contentResolver.query(
        URI_CONTENT_SMS, null,
        THREAD_ID + " = ? " + // we don't support drafts yet
            " AND " + ADDRESS + " NOT NULL ",
        new String[] {String.valueOf(threadId)}, orderClause + limitClause);
  ...
  @Nullable
  public SMSMessageCursorWrapper2
  getSMSMessagesByThreadId2(Context context, int threadId, boolean desc,
                            int limit) {
    if (!Permissions.isGranted(context, Permissions.READ_SMS) ||
        !Permissions.isGranted(context, Permissions.READ_CONTACTS)) {
      return null;
  ...
  public int getSMSMessagesUnreadCountByThreadId(Context context,
                                                 int threadId) {
    if (!Permissions.isGranted(context, Permissions.READ_SMS)) {
      return 0;
    }
    ...
    Cursor cursor = contentResolver.query(
        URI_CONTENT_SMS_INBOX, new String[] {"COUNT(" + ID + ")"},
        THREAD_ID + " = ? AND " + READ + " = ? ",
        new String[] {String.valueOf(threadId), String.valueOf(0)}, null);
  ...
  public boolean setSMSMessagesReadByThreadId(Context context, int threadId) {
    ...
    return contentResolver.update(
               URI_CONTENT_SMS_INBOX, values,
               THREAD_ID + " = ? AND " + READ + " = ? ",
               new String[] {String.valueOf(threadId), String.valueOf(0)}) > 0;
  }
  ...
  public boolean deleteSMSMessagesByThreadId(Context context, int threadId) {
    ...
    int count = contentResolver.delete(URI_CONTENT_SMS, THREAD_ID + " = ? ",
  ...
  public boolean deleteSMSMessageById(Context context, long id) {
    ...
    int count = contentResolver.delete(URI_CONTENT_SMS, ID + " = ? ",
                                       new String[] {String.valueOf(id)});
  ...
  public boolean setSMSMessagesSeen(Context context) {
    ...
    ContentValues values = new ContentValues();
    values.put(SEEN, 1);
    return contentResolver.update(URI_CONTENT_SMS_INBOX, values, SEEN + " = ? ",
  ...
  public int getSMSThreadIdByNumber(Context context, String number) {
    ...
    Cursor cursor = contentResolver.query(
        URI_CONTENT_SMS, new String[] {THREAD_ID}, ADDRESS + " = ? ",
  ...
  private class SMSMessageCursorWrapper extends CursorWrapper {
    ...
    SMSMessage getSMSMessage(boolean withContact) {
      ...
      return new SMSMessage(id, type, status, date, date_sent, person, number,
                            body);
    }
  }
  ...
  private SMSMessage getSMSMessagesById(long id, boolean withContact) {
    //        if (!Permissions.isGranted(context, Permissions.READ_SMS) ||
    //                !Permissions.isGranted(context,
    //                Permissions.READ_CONTACTS)) {
    //            return null;
    //        }

    Cursor cursor = contentResolver.query(URI_CONTENT_SMS, null,
                                          ID + " = " + id, null, null);

    SMSMessage message = null;
    if (validate(cursor)) {
      SMSMessageCursorWrapper cursorWrapper =
          new SMSMessageCursorWrapper(cursor);
  ...
  @TargetApi(19)
  public boolean writeSMSMessageToInbox(Context context,
  ...
  public long writeSMSMessageToOutbox(Context context, String number,
                                      String message) {
    if (!Permissions.isGranted(context, Permissions.WRITE_SMS)) {
      return -1;
    }
  ...
  public boolean updateSMSMessageOnSent(Context context, long messageId,
                                        boolean delivery, boolean failed) {
    int type, status;
    if (failed) {
  ...
  public boolean updateSMSMessageOnDelivery(Context context, long messageId,
                                            boolean failed) {
    int type, status;
    if (failed) {
      type = MESSAGE_TYPE_FAILED;
    ...
    return updateSMSMessage(context, messageId, type, status,
                            System.currentTimeMillis());
  }

  // Updates SMS message
  private boolean updateSMSMessage(Context context, long messageId, int type,
                                   int status, long deliveryDate) {
    if (!Permissions.isGranted(context, Permissions.WRITE_SMS)) {
      return false;
    }
    ...
    return contentResolver.update(URI_CONTENT_SMS, values, ID + " = ? ",
  ...
  // For the sake of performance we don't use comprehensive phone number
  // pattern. We just want to detect whether a phone number is digital but not
  // symbolic.
  private static final Pattern digitalPhoneNumberPattern =
      Pattern.compile("[+]?[0-9-() ]+");
  // Is used for normalizing a phone number, removing from it brackets, dashes
  // and spaces.
  private static final Pattern normalizePhoneNumberPattern =
      Pattern.compile("[-() ]");
