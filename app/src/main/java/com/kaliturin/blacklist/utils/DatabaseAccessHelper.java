...
static String getLikeClause(String column, String filter) {
	return (filter == null ? null : column + " LIKE '%" + filter + "%' ");
}
...
static final String CREATE =
	"CREATE TABLE " + JournalTable.NAME + "(" + Column.ID +
	" INTEGER PRIMARY KEY NOT NULL, " + Column.TIME +
	" INTEGER NOT NULL, " + Column.CALLER + " TEXT NOT NULL, " +
	Column.NUMBER + " TEXT, " + Column.TEXT + " TEXT "
	+ ")";

static final String SELECT_FIRST_PART =
	"SELECT " + Column.ID + ", " + Column.TIME + " FROM " +
	JournalTable.NAME + " ORDER BY " + Column.TIME + " DESC";

static final String SELECT_LAST_PART_BY_ID =
	"SELECT " + Column.CALLER + ", " + Column.NUMBER + ", " +
	Column.TEXT + " FROM " + JournalTable.NAME + " WHERE _id = ? ";

static final String SELECT_FIRST_PART_BY_FILTER =
	"SELECT * "
	+ " FROM " + JournalTable.NAME + " WHERE " + Column.CALLER +
	" LIKE ? "
	+ " OR " + Column.TEXT + " LIKE ? "
...
JournalRecord(long id, long time, @NonNull String caller, String number,
              String text) {
	this.id = id;
	this.time = time;
	this.caller = caller;
...
private String[] getJournalRecordPartsById(long id) {
	...
	if (validate(cursor)) {
		...
		parts = new String[] {cursor.getString(CALLER), cursor.getString(NUMBER),
...
public JournalRecordCursorWrapper getJournalRecords(@Nullable String filter) {
	...
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor =
		db.rawQuery(JournalTable.Statement.SELECT_FIRST_PART_BY_FILTER,
...
public int deleteJournalRecords(IdentifiersContainer contactIds,
	...
	// build 'WHERE' clause
	String clause = Common.concatClauses(
		new String[] {Common.getLikeClause(JournalTable.Column.CALLER, filter),
...
public boolean deleteJournalRecord(long id) {
	SQLiteDatabase db = getWritableDatabase();
	return (db.delete(JournalTable.NAME, JournalTable.Column.ID + " = " + id,
	                  null) > 0);
}

// Writes journal record
public long addJournalRecord(long time, @NonNull String caller, String number,
                             String text) {
...
static class Statement {
static final String CREATE =
	"CREATE TABLE " + ContactNumberTable.NAME + "(" + Column.ID +
	" INTEGER PRIMARY KEY NOT NULL, " + Column.NUMBER +
	" TEXT NOT NULL, " + Column.TYPE + " INTEGER NOT NULL, " +
...
static final String SELECT_BY_CONTACT_ID =
	"SELECT * "
	+ " FROM " + ContactNumberTable.NAME + " WHERE " + Column.CONTACT_ID +
	" = ? "
	+ " ORDER BY " + Column.NUMBER + " ASC";

static final String SELECT_BY_TYPE_AND_NUMBER =
	"SELECT * "
	+ " FROM " + ContactNumberTable.NAME + " WHERE " + Column.TYPE +
	" = ? "
	+ " AND " + Column.NUMBER + " = ? ";
...
static final String SELECT_BY_NUMBER =
	"SELECT * "
	+ " FROM " + ContactNumberTable.NAME + " WHERE (" + Column.TYPE +
	" = " + ContactNumber.TYPE_EQUALS + " AND "
	+ " ? = " + Column.NUMBER + ") OR (" + Column.TYPE + " = " +
	ContactNumber.TYPE_STARTS + " AND "
	+ " ? LIKE " + Column.NUMBER + "||'%') OR (" + Column.TYPE + " = " +
	ContactNumber.TYPE_ENDS + " AND "
	+ " ? LIKE '%'||" + Column.NUMBER + ") OR (" + Column.TYPE + " = " +
	ContactNumber.TYPE_CONTAINS + " AND "
	+ " ? LIKE '%'||" + Column.NUMBER + "||'%')";
}
}
...
public ContactNumber(long id, @NonNull String number, int type,
                     long contactId) {
	this.id = id;
	this.number = number;
	this.type = type;
...
private int deleteContactNumber(long id) {
	...
	return db.delete(ContactNumberTable.NAME,
	                 ContactNumberTable.Column.ID + " = " + id, null);
}

// Selects contact numbers by contact id
@Nullable
private ContactNumberCursorWrapper
getContactNumbersByContactId(long contactId) {
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor =
		db.rawQuery(ContactNumberTable.Statement.SELECT_BY_CONTACT_ID,
...
private ContactNumberCursorWrapper getContactNumbersByNumber(String number) {
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor = db.rawQuery(ContactNumberTable.Statement.SELECT_BY_NUMBER,
...
@Nullable
private ContactNumberCursorWrapper
getContactNumbersByTypeAndNumber(int numberType, String number) {
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor =
		db.rawQuery(ContactNumberTable.Statement.SELECT_BY_TYPE_AND_NUMBER,
...
// This method is mainly needed for retrieving actual ContactNumber.id and/or
...
static class Statement {
static final String CREATE =
	"CREATE TABLE " + ContactTable.NAME + "(" + Column.ID +
	" INTEGER PRIMARY KEY NOT NULL, " + Column.NAME + " TEXT NOT NULL, " +
	Column.TYPE + " INTEGER NOT NULL DEFAULT 0 "
	+ ")";

static final String SELECT_BY_TYPE =
	"SELECT * "
	+ " FROM " + ContactTable.NAME + " WHERE " + Column.TYPE + " = ? "
	+ " ORDER BY " + Column.NAME + " ASC";

static final String SELECT_BY_NAME = "SELECT * "
...
Contact(long id, @NonNull String name, int type,
        @NonNull List<ContactNumber> numbers) {
	this.id = id;
	this.name = name;
	this.type = type;
...
public interface ContactSource { Contact getContact(); }
...
private ContactCursorWrapper getContacts(int contactType) {
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor = db.rawQuery(ContactTable.Statement.SELECT_BY_TYPE,
...
@Nullable
public ContactCursorWrapper getContacts(int contactType,
                                        @Nullable String filter) {
	if (filter == null) {
...
public ContactCursorWrapper getContact(String contactName) {
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor = db.rawQuery(ContactTable.Statement.SELECT_BY_NAME,
...
private ContactCursorWrapper getContact(int contactType, String contactName) {
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor =
		db.rawQuery(ContactTable.Statement.SELECT_BY_TYPE_AND_NAME,
...
public ContactCursorWrapper getContact(long contactId) {
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor = db.rawQuery(ContactTable.Statement.SELECT_BY_ID,
	                            new String[] {String.valueOf(contactId)});
...
public long addContact(int contactType, @NonNull String contactName,
                       @NonNull List<ContactNumber> numbers) {
	if (numbers.size() == 0)
		return -1;
...
private void deleteContactNumbers(List<ContactNumber> numbers) {
	...
	for (Long contactId : contactIds) {
		...
		ContactNumberCursorWrapper cursor =
			getContactNumbersByContactId(contactId);
		if (cursor == null) {
			// delete it
...
public int deleteContacts(int contactType, IdentifiersContainer contactIds,
	...
	// build 'WHERE' clause
	String clause = Common.concatClauses(
		new String[] {ContactTable.Column.TYPE + " = " + contactType,
...
// Searches contacts by contact numbers (retrieving them by
// ContactNumber.contactId)
...
private int reverseContactType(int type) {
	return (type == Contact.TYPE_BLACK_LIST ? Contact.TYPE_WHITE_LIST
...
static class Statement {
static final String CREATE =
	"CREATE TABLE " + SettingsTable.NAME + "(" + Column.ID +
	" INTEGER PRIMARY KEY NOT NULL, " + Column.NAME + " TEXT NOT NULL, " +
	Column.VALUE + " TEXT "
	+ ")";
...
private SettingsItemCursorWrapper getSettings(@NonNull String name) {
	SQLiteDatabase db = getReadableDatabase();
	Cursor cursor = db.rawQuery(SettingsTable.Statement.SELECT_BY_NAME,
	                            new String[] {name});
...
public boolean setSettingsValue(@NonNull String name, @NonNull String value) {
	...
	// try to update value
	int n = db.update(SettingsTable.NAME, values,
	                  SettingsTable.Column.NAME + " = ? ", new String[] {name});
	if (n == 0) {
