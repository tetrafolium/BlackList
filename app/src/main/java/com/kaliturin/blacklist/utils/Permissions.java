...
public class Permissions {
  ...
  private static final int REQUEST_CODE =
      (Permissions.class.hashCode() & 0xffff);
  private static final Map<String, Boolean> permissionsResults =
  ...
  private static ArrayList<String> PERMISSIONS = new ArrayList<>(
      Arrays.asList(WRITE_EXTERNAL_STORAGE, RECEIVE_SMS, WRITE_SMS, READ_SMS,
                    SEND_SMS, CALL_PHONE, READ_PHONE_STATE, READ_CONTACTS,
                    WRITE_CONTACTS, READ_CALL_LOG, WRITE_CALL_LOG, VIBRATE));
   ...
   **/
  public static boolean notifyIfNotGranted(@NonNull Context context,
                                           @NonNull String permission) {
    if (!isGranted(context, permission)) {
   ...
   **/
  @Nullable
  private static String getPermissionInfo(@NonNull Context context,
                                          @NonNull String permission) {
   ...
   **/
  public static void invalidateCache() { permissionsResults.clear(); }
