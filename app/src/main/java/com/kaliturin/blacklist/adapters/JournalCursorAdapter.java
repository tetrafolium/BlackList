...
public class JournalCursorAdapter extends CursorAdapter {
  ...
  private final DateFormat yearLessDateFormat =
      Utils.getYearLessDateFormat(dateFormat);
  ...
  public JournalCursorAdapter(Context context) {
    ...
    foldSMSText =
        Settings.getBooleanValue(context, Settings.FOLD_SMS_TEXT_IN_JOURNAL);
  ...
  public void bindView(View view, Context context, Cursor cursor) {
    // get cursor wrapper
    JournalRecordCursorWrapper cursorWrapper =
        (JournalRecordCursorWrapper)cursor;
    // get journal item
    ...
    DateFormat df = null;
    // if date of the previous record isn't the same as the current one - show
    // date
    if (lastRecordDay != currentRecordDay ||
        lastRecordYear != currentRecordYear) {
  ...
  public IdentifiersContainer getCheckedItems() { return checkedItems; }
  ...
  public boolean hasCheckedItems() { return !checkedItems.isEmpty(); }
  ...
  private class ViewHolder {
    ...
    ViewHolder(Context context, ImageView iconImageView,
               TextView senderTextView, TextView textTextView,
               TextView dateTextView, TextView timeTextView, CheckBox checkBox,
               View dateLayout, CheckableLinearLayout contentLayout) {
      this.record = null;
      this.itemId = 0;
    ...
    private boolean isChecked() { return checkedItems.contains(itemId); }
    ...
    private boolean isTextUnfolded() { return unfoldedTextItems.get(itemId); }
