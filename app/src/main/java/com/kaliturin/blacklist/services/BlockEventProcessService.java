...
public class BlockEventProcessService extends IntentService {
  ...
  // Processes the event
  private void processEvent(Context context, String number, String name,
                            String body) {
  ...
  private void writeToJournal(Context context, String number,
                              @NonNull String name, String body) {
    ...
    if (db != null && db.addJournalRecord(time, name, number, body) >= 0) {
      ...
      InternalEventBroadcast.send(context,
                                  InternalEventBroadcast.JOURNAL_WAS_WRITTEN);
    }
  }
  ...
  // Starts the service
  public static void start(Context context, String number, String name,
                           String body) {
