...
public class SMSProcessService extends IntentService {
  ...
  public SMSProcessService() { super(SMSProcessService.class.getName()); }
  ...
  private void processMessageData(Context context, Map<String, String> data) {
    ...
    // if before API 19
    if (!DefaultSMSAppHelper.isAvailable() || // or if not "default SMS app"
  ...
  private Map<String, String> extractMessageData(@Nullable Intent intent)
    ...
    if (data.isEmpty()) {
      ...
      throw new IllegalArgumentException("Message intent data is illegal: " +
                                         intentString);
    }
