...
public void onReceive(final Context context, Intent intent) {
	...
	// phone number, and another with it blank. Due to the nature of broadcasts,
	// you cannot assume the order in which these broadcasts will arrive,
	// however you are guaranteed to receive two in this case. Apps which are
	// interested in the EXTRA_INCOMING_NUMBER can ignore the broadcasts where
	// EXTRA_INCOMING_NUMBER is not present in the extras (e.g. where
	// Intent#hasExtra(String) returns false).
	if (!intent.hasExtra(TelephonyManager.EXTRA_INCOMING_NUMBER)) {
		Log.d(
			TAG,
			"Event had no incoming_number metadata. Letting it keep ringing...");
		return;
	}
	...
	if (ContactsAccessHelper.isPrivatePhoneNumber(number)) {
		...
		if (Settings.getBooleanValue(
			    context, Settings.BLOCK_PRIVATE_CALLS) || // or if block all calls
...
private void breakCallNougatAndLower(Context context) {
	Log.d(TAG,
	      "Trying to break call for Nougat and lower with TelephonyManager.");
	TelephonyManager telephony =
		(TelephonyManager)context.getSystemService(Context.TELEPHONY_SERVICE);
	try {
