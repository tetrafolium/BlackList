...
public class SMSBroadcastReceiver extends BroadcastReceiver {
  ...
  private static final String SMS_DELIVER =
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    if (action == null ||
        !action.equals(SMS_DELIVER) && !action.equals(SMS_RECEIVED)) {
      return;
    }
    ...
    if (DefaultSMSAppHelper.isAvailable() && isDefaultSmsApp &&
  ...
  @Nullable
  private Map<String, String> extractMessageData(Context context, Intent intent,
                                                 long timeReceive) {
    // get messages
    ...
    // Assume that all messages in array received at ones have the same data
    // except of bodies. So get just the first message to get the rest data.
    ...
    data.put(ContactsAccessHelper.DATE_SENT,
             String.valueOf(message.getTimestampMillis()));
    data.put(ContactsAccessHelper.PROTOCOL,
             String.valueOf(message.getProtocolIdentifier()));
    data.put(ContactsAccessHelper.REPLY_PATH_PRESENT,
             String.valueOf(message.isReplyPathPresent()));
    data.put(ContactsAccessHelper.SERVICE_CENTER,
  ...
  private boolean processMessageData(Context context,
    ...
    if (ContactsAccessHelper.isPrivatePhoneNumber(number)) {
      ...
      if (Settings.getBooleanValue(
              context, Settings.BLOCK_PRIVATE_SMS) || // or if block all SMS
  ...
  private void abortSMSAndNotify(Context context, String number, String name,
                                 String body) {
